...
Public Module TagEvents
    ...
    Public Sub ReceiveEvent(ByRef ent As PRE2.Entity, listenerTag As PRE2.Tag, Optional room As FrmGame.Room = Nothing)
        ...
        For Each tag As PRE2.Tag In ent.tags
            ...
            If IsArray(temp) Then
                For index As Integer = 0 To UBound(temp)
                    TagBehaviours.ProcessTag(temp(index).GetArgument(), ent, room)
                Next
            ElseIf Not IsNothing(temp) Then
                TagBehaviours.ProcessTag(temp.GetArgument(), ent, room)
            End If
            'End If
        Next
    End Sub
